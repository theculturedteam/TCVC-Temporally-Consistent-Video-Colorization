...
class TCVC_IDC(nn.Module):
    def __init__(
        ...
        self.flow.load_state_dict(
            torch.load("../experiments/pretrained_models/FlowNet2_checkpoint.pth.tar")[
                "state_dict"
            ]
        )
        self.flow.eval()
    ...
    def forward(self, x, first_key_HR=None, first_key_fea=None):
        ...
        for i in range(len(x) - 2, 0, -1):
            ...
            flow = self.flow(
                x_current.repeat(1, 3, 1, 1) + 0.5, x_n.repeat(1, 3, 1, 1) + 0.5
            )
        ...
        x_n = x[1]
        flow_n_c = self.flow(
            x_n.repeat(1, 3, 1, 1) + 0.5, x_current.repeat(1, 3, 1, 1) + 0.5
        )
        ...
        for i in range(1, len(x) - 1):
            ...
            # print(f"x_p_fea: {x_p_fea.shape}")
            # print(f"x_c_fea: {x_c_fea.shape}")
            # print(f"x_n_fea: {x_n_fea.shape}")
            # print(f"backward_fea_l: {backward_fea_l[i-1].shape}")
            # print(f"fea_forward: {fea_forward.shape}")

            # weighting network
            W = torch.sigmoid(
                self.weigting(
                    torch.cat(
                        [x_p_fea, x_c_fea, x_n_fea, backward_fea_l[i - 1], fea_forward],
                        dim=1,
                    )
                )
            )  # [B, 1, H, W]   in_c: 32*3+128+128
            ...
            fea_residual = self.feature_refine(
                torch.cat(
                    [
                        x_p_fea,
                        x_c_fea,
            ...
            non_mask = torch.exp(
                -50 * torch.sum(x_n - warp_x_c, dim=1).pow(2)
            ).unsqueeze(1)
            non_mask_fwarp_l.append(non_mask)
        ...
        if len(x) >= 3:
            ...
            non_mask_fwarp_l_stack_2 = torch.stack(
                non_mask_fwarp_l_2, dim=1
            )  # [B, N-2, H, W]
        else:
        ...
        return (
            out,
            ab_fwarp_l_stack,
            non_mask_fwarp_l_stack,
            ab_fwarp_l_stack_2,
            non_mask_fwarp_l_stack_2,
        )
